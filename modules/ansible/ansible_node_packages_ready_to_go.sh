...
############################################################
# RHEL : Install Packages, Pip, and Ansible Collections   #
############################################################
main::install_packages() {
  ...
  if [[ ${LINUX_DISTRO} = "RHEL" ]]; then
    ...
    ## Install OS packages
    main::log_info "Installing OS packages"
    ...
    for pip_package in $GLOBAL_PIP_PACKAGES; do
      local count=0
      local max_count=3
      while ! pip3 install --upgrade "${pip_package}"; do
        count=$((count + 1))
        sleep 3
        if [[ ${count} -gt ${max_count} ]]; then
          main::log_error "Failed to install python package: ${pip_package}"
          # shellcheck disable=SC2317
          break
        fi
      done
    ...
    main::log_info "Installing Ansible Galaxy collections"
    ...
    main::log_info "All packages, pip packages, and collections installed successfully"
  fi
